 ...
 *If we messup here its gonna break  */
...
void sha256_final(struct sha256_ctx *ctx, u8 out_hash32[32]) {
    ...
    // Step 1: Padding starts with 0x80 followed by zeros
    for (i = 0; i < 64; ++i) pad[i] = 0;
    pad[0] = 0x80;

    // Step 2: Pad so message length ≡ 56 mod 64 (leaving 8 bytes for length)
    if (ctx->buflen < 56) {
        u32 padlen = 56 - ctx->buflen;
        sha256_update(ctx, pad, padlen);
    } else {
        u32 padlen = 64 - ctx->buflen;
        sha256_update(ctx, pad, padlen);
        sha256_update(ctx, pad, 56); // extra block
    }

    // Step 3: Append 64-bit message length (big-endian)
    u8 len64[8];
    u64 bits = ctx->bitlen;
    len64[0] = (u8)((bits >> 56) & 0xFF);
    len64[1] = (u8)((bits >> 48) & 0xFF);
    len64[2] = (u8)((bits >> 40) & 0xFF);
    len64[3] = (u8)((bits >> 32) & 0xFF);
    len64[4] = (u8)((bits >> 24) & 0xFF);
    len64[5] = (u8)((bits >> 16) & 0xFF);
    len64[6] = (u8)((bits >> 8) & 0xFF);
    len64[7] = (u8)(bits & 0xFF);

    sha256_update(ctx, len64, 8);

    // Step 4: Output final digest in big-endian
    for (i = 0; i < 8; ++i) {
        u32 val = ctx->h[i];
        out_hash32[i*4 + 0] = (u8)((val >> 24) & 0xFF);
        out_hash32[i*4 + 1] = (u8)((val >> 16) & 0xFF);
        out_hash32[i*4 + 2] = (u8)((val >> 8) & 0xFF);
        out_hash32[i*4 + 3] = (u8)(val & 0xFF);
    }
}
